...
export default function RefundPolicyPage() {
  ...
  // Fetch the current refund policy content
  useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaveMessage('Refund policy updated successfully!');
      setTimeout(() => setSaveMessage(''), 3000);
    } catch (error) {
      console.error('Error saving refund policy:', error);
      setSaveMessage('Error saving refund policy. Please try again.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handlePreview = () => {
    ...
    // For simplicity, we'll just navigate to the refund page
    router.push('/refund');
  };
    ...
    <BackendLayout activePage="content">
      <div>
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-2xl font-bold">{t('refund_policy', 'Refund Policy')}</h1>
            <p className="text-gray-600">{t('refund_policy_description', 'Manage your website refund policy content')}</p>
          </div>
