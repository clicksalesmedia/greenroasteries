      ...
      try {
        ...
        const captureData = {
          ...
          items: paymentDetails.order?.items?.map((item: any) => ({
            ...
            reference_id: item.reference_id || `item_${Date.now()}`,
            image_url: item.image_url || "https://thegreenroasteries.com/images/placeholder.jpg",
            product_url: item.product_url || "https://thegreenroasteries.com/shop",
            ordered: item.quantity || 1,
            captured: item.quantity || 1,
            shipped: 0,
            refunded: 0,
        ...
        // Update payment record to indicate capture was attempted
        await prisma.payment.updateMany({
          where: { tabbyPaymentId: payment.id },
          data: {
            status: 'PROCESSING',
